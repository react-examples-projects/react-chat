...
export default function useCalculateReactionPosition(
  ...
  useEffect(() => {
    const isReadyNodes = textChatBoxRef.current && userReactionsRef.current && reactionSelectorRef.current;
    ...
    const reactionSelectorRect = reactionSelectorRef.current.getBoundingClientRect();

    const widthTextChat = Math.trunc(textChatRect.width);
